...
if [[ "$skip_openssl" = 'no' || "$1" = 'openssl' ]]; then
    #
    download_file "openssl" "https://github.com$(curl -sNL https://github.com/openssl/openssl/releases | grep -Eo '/openssl/openssl/archive/(.*).tar.gz' | sort -V | tail -1)"
    #
...
if [[ "$skip_lftp" = 'no' ||  "$1" = 'lftp' ]]; then
    #
    download_file "lftp" "$(curl -sNL http://lftp.yar.ru/get.html | grep -Eom1 'http://lftp.yar.ru/ftp/lftp-([0-9]{1,}[.]?)([0-9]{1,3}[.]?)([0-9]{1,3}?).tar.gz')"
    #
